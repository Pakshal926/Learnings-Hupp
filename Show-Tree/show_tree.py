...
def display_tree(directory, prefix="", output_file=None, exclude=(".git", "__pycache__")):
    ...
    for index, item in enumerate(items):
        ...
        is_last = index == (total_items - 1)  # Check if this is the last item in the current directory

        # Determine which prefix to use for the current item
        connector = "└── " if is_last else "├── "
        output_file.write(prefix + connector + item + ("/" if os.path.isdir(path) else "") + "\n")
